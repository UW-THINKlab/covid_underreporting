...
I_max <- 4172
#E_0 <- 7
I_0 <- 22 #412    #### modeler assumes 25% of the cases were not reported,
R_0 <- 23 #345    #### look up in the simulation study design document Table X
...
# parameters <- c(beta_para=beta_obj, latent_para=5.1, gamma_para=0.05)
# states <- c(S=100000-I_0-R_0-E_0, E=E_0, I=I_0, R=R_0)
# 
# SEIR <- function(t, states, parameters) {
#   with(
#     as.list(c(states, parameters)),
#     {
#       dS <- -beta_para*S*I/N
#       dE <- beta_para*S*I/N - E/latent_para
#       dI <- E/latent_para - gamma_para*I
#       dR <- gamma_para*I
#       
#       list(c(dS, dE, dI, dR))
#     }
...
for (ti in 1:730) {
  ...
  #     {
  #       dS <- -beta_para*S*I/N
  #       dE <- beta_para*S*I/N - E/latent
  #       dI <- E/latent - gamma_para*I
  #       dR <- gamma_para*I
  #       
  #       list(c(dS, dE, dI, dR))
  #     }
...
#write.csv(dataset_baseline, "G:/My Drive/NIH_guan/coronavirus/US study/distributable version/alternative policies/SIR_version/baseline.csv", row.names = F)
write.csv(dataset_baseline, "/Users/gracejia/Documents/A-UW/covid19 NSF Project/review_comment_Mar12/baseline.csv", row.names = F)
...
par(new=T)
#plot(E_t,type="l",ylim=c(0,100000),lwd=2,col="orange",yaxt="n",xaxt="n",ylab="",xlab="")
#par(new=T)
...
#write.csv(dataset_scenario, "G:/My Drive/NIH_guan/coronavirus/US study/distributable version/alternative policies/SIR_version/scenario2_075.csv", row.names = F)
write.csv(dataset_scenario, "/Users/gracejia/Documents/A-UW/covid19 NSF Project/review_comment_Mar12/scenario2_075.csv", row.names = F)
plot(beta_t, type="l", xlab="Days forward (from May 15)",ylab="",cex.lab=1.4)
par(new=T)
