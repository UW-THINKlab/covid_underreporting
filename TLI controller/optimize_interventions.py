...
def S_of_I(I, R0, S0 = 1, I0 = 0):
    ...
    # Get the maximum value of I --> Ipeak
    ...
    exponent = R0 * (I - I0 - S0)
    # lambertw(NUM) returns the value z s.t. z * exp(z) = NUM
...
def I_max(S, I, R0):
    # The maximum value of I this moment onwards
    return (S + I - (1/R0) * np.log(S) -
            (1/R0) + (1/R0) * np.log(1/R0))

def I_max_after_opt_intervention(S_i, I_i, f, R0, gammatau):
    # Eq 12 13
    # S_i is the state S when intervention begins, S_f is the state S when suppression begins
    # returns the Eq 18, the maximum value of I achieved during the periof of t_f to infinity
...
def I_max_opt_of_S_i(S_i, f, R0, gammatau):
    I_i = I_of_S(S_i, R0)
    # the maximum value is either the one achieved in the future under intervention 
    # or the current value without intervention
    return max(
...
def calc_Sf_opt(R0,
    ...
    # select the S_i_expected and f that minimizes log(I_max)
...
def calc_f_opt(R0,
    ...
    if t_i is not None and gamma is not None:
        # original code said S_i = S_of_t(S, R0, gamma)
        # changed to S_i = S_of_t(S_i, R0, gamma) ?
        S_i = S_of_t(S_i, R0, gamma)
    elif S_i is None:
    ...
    #originally minimize_scalar(log_Imax_opt , ...)
    #changed to  (log_Imax_opt_f, ...)
...
def Imax_of_S_i_b(S_i,
    ...
    # The S and I states during the course of intervention
    intervention = odeint(deriv, [S_i, I_i],
                          np.linspace(0, tau,
                                      (1 + int(tau)) * 1000))
    # Max I achieved during this time
    I_max_interv = np.max(intervention[:, 1])
    # final states when intervention ends
    S_f, I_f = intervention[-1]
    # The maximum value of I this moment onwards (after the intervention)
    I_max_f = I_max(S_f, I_f, R0)
    # return the maximum among start I, I during the intervention, and the I afterwards
